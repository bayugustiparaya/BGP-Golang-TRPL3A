...
import (
	...
	cm "BGP-Golang-TRPL3A/HtmlPage/common"
...
func Index(w http.ResponseWriter, r *http.Request) {	
	...
	for result.Next() {
		...
		err := result.Scan(&employee.EmployeeID, &employee.LastName, &employee.FirstName, &employee.Title, &employee.TitleOfCourtesy, &employee.BirthDate,
			&employee.HireDate, &employee.Address, &employee.City, &employee.Region, &employee.PostalCode, &employee.Country,
			&employee.HomePhone, &employee.Extension, &employee.Photo, &employee.Notes, &employee.ReportsTo, &employee.ProvinceName)
		if err != nil {
			panic(err.Error())
		}
